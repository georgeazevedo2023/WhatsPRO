...
| [[log.md]] | **Sessão atual 2026-05-04** — Auditoria Admin (nota 6.5/10) + Sprint 1+2 admin + R90 hotfix user_roles UNIQUE + **D30 Fila Inteligente Sprints A+B shipped** (6 migrations + RPC `pick_next_assignee` + `_shared/handoffQueue` + edge fn `assign-handoff` + 6 paths em ai-agent integrados, deploy pendente) |
...
| [[wiki/casos-de-uso/handoff-fila-detalhado]] | 11 | **D30 (Sprints A+B shipped 2026-05-04, deploy pendente)** Fila Inteligente: schema DB + RPC atômico + backend completo em ai-agent (6 paths via closure runQueueAssignment com try/catch). Sprints C-H pendentes. 2 modos (ON/OFF), round-robin global, timeout 5min, pausa horário comercial, drag-drop ordem, toggle pause individual, override gestor |
...
*Última atualização: 2026-05-04 (D30 Fila Sprints A+B) — Estado atual: PRD v7.22.0, M19 ativo (S6/S7/S9 abertos). Última sessão: **D30 Fila Inteligente — Sprints A+B** (DB completo + backend completo: closure `runQueueAssignment` integrada nos 6 paths de `ai-agent/index.ts` via try/catch fallback, edge fn `assign-handoff` para cron+helpdesk Sprints C/F). Deploy edge fns pendente. Sprints C-H pendentes. Anteriores: Auditoria Admin (Sprint 1+2 + R90 hotfix), Helpdesk top tabs ESCOPO/mobile-first/equipe deptos inline. Bundle prod `index-CFmkOcne.js`, ai-agent v173 (será v174 após deploy do Sprint B).*
