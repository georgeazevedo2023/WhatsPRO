...
| [[log.md]] | **Sessão atual 2026-05-04** — Auditoria Admin (nota 6.5/10) + Sprint 1+2 admin + R90 hotfix user_roles UNIQUE + **D30 Fila Inteligente A+B+C em prod + D admin UI shipped** (3 edge fns + cron + QueueConfig dialog + select default_dept; push pendente para o frontend) |
...
| [[wiki/casos-de-uso/handoff-fila-detalhado]] | 11 | **D30 (Sprints A+B+C+D shipped 2026-05-04)** Fila Inteligente completa em backend + admin UI: schema DB + RPC atômico + ai-agent v174 + cron requeue v1 + `QueueConfig.tsx` dialog (toggle/slider/drag-drop) + select default_dept em InboxesTab. Sprints E-H pendentes. 2 modos (ON/OFF), round-robin global, timeout 5min, pausa horário comercial, drag-drop ordem, toggle pause individual, override gestor |
