...
| [[log.md]] | **Sessão atual 2026-05-04** — Auditoria Admin (nota 6.5/10) + Sprint 1+2 admin + R90 hotfix user_roles UNIQUE + **D30 Fila Inteligente A+B em prod + C code-complete** (ai-agent v174 + assign-handoff v1; requeue-conversations + cron migration aguardam deploy) |
...
| [[wiki/casos-de-uso/handoff-fila-detalhado]] | 11 | **D30 (Sprints A+B em prod, C code-complete 2026-05-04)** Fila Inteligente: schema DB + RPC atômico + backend ai-agent (6 paths) + cron `requeue-conversations` (5 cases + reativação Q5) — C aguarda deploy. Sprints D-H pendentes. 2 modos (ON/OFF), round-robin global, timeout 5min, pausa horário comercial, drag-drop ordem, toggle pause individual, override gestor |
