...
| [[log.md]] | **Sessão 2026-05-05** — D30 Fila Inteligente Sprint G (53 testes Vitest novos + retention policy seed `handoff_queue_events` 90d em prod). 715 testes passam (+53), 0 regressão. tsc 0. Sprints E (modo estendido), H (docs) pendentes. |
...
| [[wiki/casos-de-uso/handoff-fila-detalhado]] | 11 | **D30 (Sprints A+B+C+D+F+G shipped 2026-05-04 a 05-05)** Fila Inteligente completa: backend + admin UI + helpdesk UI (badge "Em fila — Lucas (3:42)" countdown ao vivo + pause toggle pessoal + cancelar fila em reatribuição manual) + 53 testes Vitest + retention policy 90d. Sprints E (modo estendido), H (docs) pendentes. 2 modos (ON/OFF), round-robin global, timeout 5min, pausa horário comercial, drag-drop ordem, toggle pause individual, override gestor |
