...
| [[log.md]] | **Sessão atual 2026-05-04** — Auditoria Admin (nota 6.5/10) + Sprint 1+2 admin + R90 hotfix user_roles UNIQUE + **D30 Fila Inteligente A+B+C em prod** (ai-agent v174 + assign-handoff v1 + requeue-conversations v1 + cron jobid 12 ativo + hotfix R92 vault.SUPABASE_ANON_KEY) |
...
| [[wiki/casos-de-uso/handoff-fila-detalhado]] | 11 | **D30 (Sprints A+B+C em prod 2026-05-04)** Fila Inteligente: schema DB + RPC atômico + backend ai-agent v174 + cron `requeue-conversations` v1 (jobid 12 ativo, 5 cases + reativação Q5, hotfix R92 vault). Sprints D-H pendentes. 2 modos (ON/OFF), round-robin global, timeout 5min, pausa horário comercial, drag-drop ordem, toggle pause individual, override gestor |
