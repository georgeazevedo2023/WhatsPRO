...
import { sanitizeAgentResponse } from './responseSanitizer.ts'
...
// =============================================================================
// Backstop de validação (2026-05-26 → Onda 2 2026-06-12)
// =============================================================================
//
// A lógica de enforcement (SAFE_TEXT/AUTO_FIX/premium fallback) foi EXTRAÍDA pra
// _shared/agent/responseSanitizer.ts — agora é a MESMA pro router (aqui) e pro
// monolith fallback (index.ts), fechando o crítico #1 da auditoria 2026-06-12
// (validação divergente entre os 2 caminhos). Histórico e política de enforcement:
// ver o header do responseSanitizer.ts.
...
/** Adapter: SpecialistCtx → SanitizerCtx neutro (lógica fonte única no responseSanitizer). */
function sanitizeSpecialistResponse(responseText: string, ctx: SpecialistCtx) {
  return sanitizeAgentResponse(responseText, {
    outgoingTexts: extractOutgoingTexts(ctx.geminiContents),
    leadName: (ctx.leadProfile?.full_name || '').trim() || null,
    toolCallsLog: ctx.toolCallsLog,
    incomingText: ctx.incomingText || null,
