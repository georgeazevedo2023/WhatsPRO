...
## 🎯 Andamento do Plano Orquestrador — **~98% concluído** (router + 5 specialists 100% em PROD nos 3 agentes; memória longa; auditoria de estrutura fechada; falta só D6 aposentar o monolito, gate ~23/06)
...
| D6 aposentar monolito (após 30d router estável; **orchestrator/Fluxos v3.0 removidos v7.90.0 + schema DB dropado 2026-06-19**) | ⏳ STAGED (monolith pendente, gate ~23/06) | — | — |
...
**Hoje (2026-06-25):** **`except_keywords` — a mangueira da máquina de lavar parou de ser recusada (v7.96.0).** Sessão ultracode+workflow. Queixa do dono (print da Fila): lead pediu *"mangueira de saída de água da máquina de lavar"* (peça hidráulica que VENDEMOS) e a IA recusou (*"Esse não é o nosso forte aqui…"*). **Causa-raiz (workflow 10 agentes, verificada em prod):** o matcher de `excluded_products` roda **PRÉ-LLM** (`index.ts:1497`) e a keyword `máquina de lavar` (item `eletrodomesticos`) casava whole-word DENTRO da frase da mangueira → short-circuit pra recusa, pulando router/qualificação/transbordo (todos verificados SÃOS — só atropelados). **Fix sem gambiarra:** campo opcional `except_keywords[]` no `excluded_products` → suprime a recusa do item quando o lead cita uma "palavra que libera" (mangueira/engate/cano/registro/válvula/torneira/sifão/…); aparelho cru (*"quero uma máquina de lavar"*) segue recusando. Mesmo whole-word/accent-insensitive (`containsWholeWord`). **Sem migration/regen-types** (JSONB livre). Decisões do dono (AskUserQuestion): abordagem=except_keywords, vocabulário=lista revisada, catálogo=só transbordo. Deploy: ai-agent **v270** + UPDATE config 3 agentes (`174af654`/`1062059a`/`9c71f43e`) + UI "Exceções" no `ExcludedProductsConfig` + frontend push→CI. tsc 0 · vitest **1939/0** (+13) · deno 0 · **smoke real 7/7** (matcher deployado × config viva). Detalhe: memória `project_except_keywords_v796`. **Próximo:** D6 monolito (gate ~23/06); lint restante (any/exhaustive-deps); god files.
