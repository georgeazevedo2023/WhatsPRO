...
**Hoje (2026-05-24, noite):** **Carousel batching "mais opções" (v7.49.0)** — primeiro gap premium dos 3 cenários consultivos (21.27-21.29). Lead rejeita carrossel/pede mais → lote NOVO excluindo os já vistos (`conversations.shown_product_ids`); cap 5 cards/lote; quando esgota, oferece refinar/categoria/consultor sem inventar. Decisão arquitetural registrada: **vector/embeddings NÃO é necessário** (catálogo bounded + funil de qualificação já entrega facetas → busca facetada > embeddings). **2 bugs raiz achados no E2E** (query sem `id`; `conversations` sem `shown_product_ids` no select) corrigidos na fonte. **E2E sandbox router 3 estados nota 10** (lote1 5 cards → lote2 2 produtos DIFERENTES → esgotado gracioso). 366 testes verdes, deno 0, deploy CLI. Andamento **~91%**. Backlog premium: #2 cart engine, #3 refino-por-contagem, #4 modo consultivo, #5 busca facetada, #6 catálogo.
