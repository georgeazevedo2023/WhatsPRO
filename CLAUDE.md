...
**(histórico anterior):** blocos de sessão de 2026-06-26 e anteriores em [[wiki/claude-historico-sessoes]]
...
| Deploy frontend | `git push origin master` → GitHub Actions → GHCR → redeploy do stack "whatspro" (Portainer, Hetzner CX42; webhook na memória `reference_portainer_webhook`). ⚠️ **a conferir (2026-07-26):** o workflow tem um step *"Redeploy via Portainer webhook"* que rodou com sucesso nos 3 commits de 25/07 e o bundle novo subiu sozinho — mas ninguém leu o YAML ainda; **confirmar em `.github/workflows/` se o webhook manual ainda é necessário** (até lá, na dúvida, verificar o bundle live antes de dizer "deployado") |
