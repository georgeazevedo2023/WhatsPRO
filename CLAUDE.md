...
**Hoje (2026-06-12):** sessão v7.85→v7.88 — (1) telefone do lead na lista do Helpdesk + fix "Garagem" (IA gravava INTERESSE como nome; `sanitizeProfileName` fonte única + 5 leads corrigidos); (2) **vaga de emprego** determinístico (`jobVacancy.ts` ANTES do gate router + `business_info.jobs_email`, dppeletropiso@hotmail.com nos 3 agentes); (3) gestores fora do drawer Reatribuir (RPC `is_manager`) + buraco D-β da fila fechado; (4) **auditoria completa do módulo AI Agent (nota 8/10)** + Onda 1 executada (constantes únicas, llmProvider repassa modelo resolvido, hot path -100ms, índice composto). Achados-chave: validador DIVERGE monolith×router; orchestrator/ é código MORTO (use_orchestrator=false nas 3); router real = gpt-4.1-mini. **Próximo: Onda 2 (validação unificada + exit_action nos specialists + HUMANIZATION_RULES única) → Onda 3/D6 (aposentar monolith ~2150 lin + deletar orchestrator + carrossel paralelo).** Pendências de DECISÃO do dono: openai_api_key por agente (ler ou remover da UI), extraction_address_enabled, specialist_model na UI, Televendas com role gerente. Detalhe: memória `project_session_2026_06_12_handoff`.
