...
**Hoje (2026-08-12→13, sessão longa):** 📷 **Envio de foto mobile CONSERTADO em 3 atos** — v7.118 `materializeFile` na entrada (File `content://` do WebView é ONE-SHOT; 2ª leitura morria com NotReadableError) + upload por fetch CRU com retry; v7.119 **cadeia inteira fora do auth client** (GoTrueClient trava na volta do picker; proxy/INSERT/UPDATE crus com token do localStorage — mesmo desenho do APK do agro, que funciona); v7.119.1 **bolha otimista** (a linha do INSERT pinta local; o refetch travado dava reload automático = "só aparece após reload"). E2E Playwright em cada ato; dono confirmou em campo. 🔔 **Push do APK validado em campo** (o "não chegou" era design: conversa do dept Vendas sem atribuição → destinatários = 9 vendedores SEM APK; rollout real = instalar APK nos vendedores). ⚡ **Perf do Helpdesk: auditoria medida em prod + 4 levas** — v7.120 (anti ping-pong do clique 11→2 requests, 1ª carga 4,0→2,7s, tick 1s condicional, `ai_summary` fora da lista; hotfix v7.120.1 useRef-sem-import, 8min exposição); v7.121 (mídia de bucket PÚBLICO sem sign — zero rede/foto, 400s do console mortos); v7.122 (bolha do payload realtime, 1-query pra conversa nova, debounce da fila, **undef-gate DURO no CI** que na estreia achou a **enquete em massa QUEBRADA** — `sendPoll` com refs inexistentes desde M17 F4, corrigido); v7.123/.1 (**lista de mensagens VIRTUALIZADA** react-virtuoso: 297 msgs → 13-16 nós DOM, prepend sem pulo, abre no fim mesmo com fotos async). Detalhe: memórias `project_mobile_photo_send_v7118` + `project_helpdesk_perf_audit_v7120` + handoff `project_session_2026_08_13_handoff`.

**(histórico) 2026-08-11 (noite):** 🚑 **v7.116.0 (`d162b1b`)** — bug GRAVE: `.maybeSingle()` sem `.limit(1)` no webhook criava conversa nova POR MENSAGEM com 2+ abertas (93 duplicadas/8 contatos; re-saudação a lead conhecido). Fix: limit(1)+erro logado, 85 resolvidas, índice único `uq_conversations_open_per_contact` + retry-23505. + foto do APK content://-safe. 🔔 **v7.117.0 (`008164e`)** — push "cliente esperando" completo (push_devices, `push-queue-alert` cron 2min anti-ruído, appPush no AuthContext, APK v1.1-build2); **aguarda SÓ o Firebase do dono** (checklist entregue). 📸 **v7.115.0 (`909c029`)** — "Tirar foto" no menu de anexos do Helpdesk (câmera direta via `capture="environment"`; visível só APK/mobile; sem novo APK — casca remota) + senha admin rotacionada (E2E dupla, valor só no `.env.local`). 👁️ **v7.114.0 (`4162a4c`)** — toggle mostrar/ocultar senha no Login (botão-olho, a11y, tsc 0 · vitest 2031/0). **+** 📱 **v7.113.0 (commit `dbb4d80`) — APP Android do vendedor (Inbox+Fila): casca Capacitor + APK assinado no CI.** `mobile/` novo: Capacitor 7 em modo `server.url` remoto (`crm.wsmart.com.br`) — câmera/galeria/mic nativos reusando 100% o pipeline de mídia do painel (ChatInput + useAudioRecorder + downscale v7.111); receita herdada de `C:\projetos\Claude\casa_do_agricultor\mobile\` (mesmo caso de uso, provada em campo), incl. o bloco `<queries>` IMAGE_CAPTURE obrigatório (targetSdk 35 — sem ele a câmera não abre em parte dos aparelhos). Build/assinatura SÓ no CI (workflow `apk-whatspro`; keystore PKCS12 openssl local gitignored + secrets no Actions). Build 1 success → `C:\projetos\claude\apks\whatspro-v1.0-build1.apk` (3,8 MB, release-signed). Falta: teste no celular real; fase 3 = push FCM. Fix colateral: `check-md-length.sh` ignora `*/node_modules/*`. Detalhe: [[wiki/app-android]]. **+ fix de dado em prod:** lead 558781104185 tinha `full_name="Deus"` (greeting capturou vocativo de "Bom dia, Deus abençoe" via padrão saudação+nome do `nameCapture.ts`) → corrigido pra "Ivonete"; fix de raiz proposto (vocabulário religioso em `NON_NAME_WORDS`) aguardando OK.

**(histórico) 2026-07-27 (cont.):** 🎧 **v7.112.0 (commit `39297c5`) — áudio do vendedor vira TEXTO + Fila com player/transcrição + foto do chat 50% menor.** (1) Foto no chat: teto **480px** (metade dos 955px medidos ao vivo); teto em PIXEL — `min(480px,100%)` não encolhe a bolha (percentual não conta na largura intrínseca). (2) `ConversationModal` (Fila/gestão): `[audio]` → `AudioPlayer` + 📝 transcrição + `(sem transcrição)` explícito, mídia via `useMediaUrl` com `instance_id` (URL crua do UAZAPI não abre) — **fotos do cliente voltaram a renderizar** de brinde; +5 testes. (3) **Áudio de saída transcrito** (era 0/114 em 30d — webhook só tratava `incoming`; o vendedor responde por áudio pelo celular): webhook liberado + **guard na FONTE** no `transcribe-audio` (lê `direction` do próprio row; só chama `ai-agent-debounce` se `incoming`, senão a IA responderia ao vendedor). Guard provado em prod (transcreveu, `ai_agent_runs` 2→2). Backfill 7d **26/37** — os 11 restantes 404: ⚠️ **URL de mídia do UAZAPI expira em ~4-5 dias e áudio é o ÚNICO tipo não espelhado no Storage** (histórico de áudio morre; espelhar no ingest = pendência recomendada). tsc 0 · deno 0 · vitest **2031/0** · build ✓. Ordem de deploy: guard ANTES do webhook.

**(histórico) 2026-07-27:** 📷 **v7.111.0 (commit `2b0e54d`) — envio de imagem no Helpdesk CONSERTADO** (os 4 fixes da auditoria de 26/07, dono deu OK): (1) downscale client-side pré-upload (JPEG/PNG >1MB → ≤2048px q0.85, best-effort "nunca pior"; HEIC pós-conversão também; PNG→JPEG fundo branco); (2) upload-timeout ≠ sessão-zumbi (`uploadTimeoutPolicy.ts`: sonda decide — sessão válida = falha limpa SEM reload, retry vivo; `unknown` = recover; `dead` = signOut declarativo); (3) AVIF/BMP nos magic bytes (AVIF não vira mais DOCUMENTO mudo); (4) telemetria de SUCESSO (`done/success` + duração + tamanho) + `sendBeacon`; migration CHECKs + edge fn `log-send-failure` v2. **E2E real na PROD via Playwright:** JPEG 7,4MB 4000×3000 → enviado 6,6s, Storage 2048×1536/1,45MB (−80%), telemetria `success (downscaled)`, UAZAPI confirmou. tsc 0 · vitest **2026/0** · build ✓ · deno 0. **Achados:** `.git/index.lock` órfão (26/07 21:42) tinha abortado o commit de docs da v7.110.0 — vault estava staged sem commit, fechado em `1b88fd1`; `deploy.yml` CONFIRMADO com redeploy automático via webhook Portainer (caveat removido da tabela de deploy). **Monitorar:** taxa success×falha por plataforma na `media_send_telemetry` (agora tem denominador). Detalhe: memória `project_helpdesk_image_fixes_v7111`.
