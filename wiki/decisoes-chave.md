...
## D30 — Fila Inteligente de Handoff (Sprints A+B+C em prod 2026-05-04)

2 modos por departamento (Q1): **ON** = round-robin global com cursor atômico, timeout 5min, loop infinito + sino gestor. **OFF** = 100% vai pro `default_assignee_id`. Modelo C visibilidade: badge "Em fila — Lucas (3:42)" mas só assignee responde. Horário comercial: pausa relógio + auto-envia `out_of_hours_message`. Toggle "Expediente Estendido" + calendário exceções. Pause individual. **D-α** fallback dept profile→funnel→`inboxes.default_department_id`→falha. **D-β** re-handoff respeita histórico (reusa último `assigned_to` se elegível). **D-γ** variável `{handoff_assignee_name}` em `agent.handoff_message`/`profile.handoff_message`/`funnel.handoff_message` substituída pelo primeiro nome do atendente. **A**: 6 migrations + RPC `pick_next_assignee` atômica. **B**: helpers `handoffQueue` + `handoffDepartment` + edge fn `assign-handoff` + integração 6 paths ai-agent via closure `runQueueAssignment` (try/catch HIGH RISK mitigado). **C**: helper `businessHours.ts` + edge fn `requeue-conversations` v1 (cron 1min jobid 12 ativo, 5 cases A-E + reativação Q5) + Realtime broadcast `queue-update`. **Hotfix R92**: vault.SUPABASE_ANON_KEY atualizada para `sb_publishable_*` (todos os crons estavam silenciosamente 401ando). **3 fns em prod** (ai-agent v174, assign-handoff v1, requeue-conversations v1). Smoke ao vivo OK. Sprints D-H pendentes. Detalhes: [[wiki/casos-de-uso/handoff-fila-detalhado]].
