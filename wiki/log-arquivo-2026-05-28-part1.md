---
title: Log arquivo 2026-05-pre-28 part1
type: log-arquivo
description: Entries 2026-05-26 e anteriores arquivadas em 2026-05-28
updated: 2026-05-28
...
**Ajuste pós-ship (v7.56.1, mesma sessão):** dono pediu pra agir só em horário comercial → gate `isOutsideBusinessHours` no cron (fora de hora espera reabrir às 8h) + janela do RPC 12h→36h (overnight sobrevive). **Feature LIGADA no EletropisoV2** (5min/+10min, msg "Ainda tá por aí?..."). Demais agentes OFF. **Pendência:** monitorar 1º abandono real; stage-1 com WhatsApp real não disparado ao vivo (jid fake no teste) — send path verbatim do `requeue-conversations`. Backlog premium: #4 modo consultivo, #5 busca facetada.
