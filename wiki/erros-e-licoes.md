...
| 84 | Sempre que adicionar categoria nova ao `service_categories` JSONB, **lembrar de expandir `VALID_KEYS` no handler `set_tags`** + deploy edge function. Não basta UPDATE no banco. Idealmente: migrar pra `VALID_KEYS` dinâmico (lê do schema da categoria) — elimina acoplamento manual. | AI Agent |
