...
- Agent 1 → `_shared/promptRules.ts` (NOVO): exporta `buildPromptRulesString()` com 5 regras de tom. 937 chars. 3/3 tests.
- Agent 2 → `_shared/responseValidator.ts` (NOVO): `validateLLMResponse(text, ctx)` com 7 regras determinísticas. 185 lin. **Modo telemetria**. 19/19 tests.
- Agent 3 → `_shared/searchGuard.ts` (estendido): nova `detectIncomingSearchSignal` cobre R121 + brand→search. +91 lin. 28/28 tests. **Não wirado** (Edit 3 = ALTO RISCO, fica pro Sprint B5).
- Agent 4 → `_shared/handoffGuard.ts` (estendido): `shouldBlockHandoffForPayment` + `mentionsPaymentTopic`. +87 lin. 23/23 tests. **Wirado** em `case 'handoff_to_human'`.
- Agent 5 → wire plan em `/tmp/B1_WIRE_PLAN.md` (4 edits, 7 riscos).
- Auditor (Agent 6): PASS COM RESSALVAS (ressalvas esperadas).

**Wire em `ai-agent/index.ts`:**
1. Import dos 4 helpers
2. Declaração `hardcodedRules` REMOVIDA (era 25 lin / 9.3 KB)
3. `systemPrompt` usa `buildPromptRulesString()`
4. `case 'handoff_to_human'` chama `shouldBlockHandoffForPayment` ANTES da lógica
5. `validateLLMResponse` em telemetria antes do validator LLM
6. `validatorAgent.ts` prompt estendido com 4 regras órfãs

**Pipeline:** tsc 0 erros, vitest 913 pass / 9 fail pré-existentes. +50 testes novos B1 pass. Deploy `ai-agent v74→v75` ACTIVE.

**Riscos/follow-up:**
- Edit 3 (searchGuard PRÉ-LLM wire) pulado: defer Sprint B5.
- `responseValidator` em telemetria 1-2 sem antes de enforcement.
- B2/B3/B4/B5 pendentes.
