...
| **D30 Fila Inteligente — Sprint B** (Backend) | ✅ Shipped (2026-05-04, deploy pendente) | `_shared/handoffQueue.ts` + `_shared/handoffDepartment.ts` + edge fn `assign-handoff` (verify_jwt=false + verifyCronOrService) + integração nos 6 paths em `ai-agent/index.ts` via closure `runQueueAssignment` com try/catch (HIGH RISK mitigado por fallback). D-α (cascata profile→funnel→inbox), D-β (re-handoff respeita último assignee se elegível), D-γ (`{handoff_assignee_name}` substituído por primeiro nome do atendente). tsc 0 erros, vitest 662 passam (5 falhas pré-existentes em FormBuilder). Sprints C–H pendentes. |
