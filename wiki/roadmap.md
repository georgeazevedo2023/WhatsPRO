...
| **D30 Fila Inteligente — Sprint C** (Cron + Horário) | ✅ Code-complete (2026-05-04, deploy pendente) | `_shared/businessHours.ts` (`isOutsideBusinessHours` extraído do ai-agent) + edge fn `requeue-conversations` (cron 1min, 5 cases A-E + reativação Q5 com 5min completos) + migration `pg_cron` (idempotente) + Realtime broadcast `queue-update`. Notifica gestores (super_admin + gerente) em loop completo + sem atendente elegível. tsc 0, vitest 662 (5 pré-existentes). Sprints D-H pendentes. |
