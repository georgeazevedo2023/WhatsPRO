...
| **M18 Fluxos v3.0** | ⚠️ Runtime descontinuado (v7.90.0) | Construído (12/12 sprints, 2026-04-12) mas **nunca ativado em prod** (`use_orchestrator=false` em tudo, 0 flow_states) e superado pelo router do ai-agent → `orchestrator` + UI `/flows` removidos em 2026-06-13. Histórico: [[wiki/fluxos-roadmap-sprints]] |
...
**Plano completo (histórico):** plano-enquetes-polls (arquivado/removido)
...
| S9 | Hardening RLS Permissões Helpdesk | 🟡 Core enforçado (2026-06-17) | **CORRIGIDO na auditoria 2026-06-17:** o backend RLS JÁ enforça o escopo via `can_view_conversation(uid, inbox_id, department_id)` — inbox obrigatório + (super_admin OU admin/gestor da inbox OU `can_view_all` OU membro do dept). **NÃO há vetor multi-tenant** (atendente não lê conversa fora da inbox/dept via API). Falta só o REFINO dos toggles granulares `can_view_unassigned`/`can_view_all_in_dept` (P2, dentro de um dept já autorizado) — não é mais risco de segurança. |
...
| **D30 Fila Inteligente — Sprint D** (Admin UI) | ✅ Shipped (2026-05-04) | `QueueConfig.tsx` dialog em DepartmentsTab: toggle Modo Fila, slider Timeout (1-15min), select Atendente Padrão, drag-drop dos membros (`@dnd-kit/sortable`, queue_position espaçada por 10), toggle Pausar/Despausar, toggle "Incluir gestor" (só renderizado para role gerente). Reset cursor RR ao salvar. Audit log `update_dept_queue_config`. **InboxesTab**: select inline "Departamento padrão (handoff)" auto-save → `inboxes.default_department_id` (D-α), audit log `set_inbox_default_dept`. tsc 0, vitest 662 (5 pré-existentes). Validado por Playwright: 7/7 cenários PASS. |
| **D30 Fila Inteligente — Sprint F** (Helpdesk UI) | ✅ Shipped (2026-05-05) | Hook `useActiveQueueEvents` (tick 1s + Realtime `queue-update`) + `QueuePauseToggle` no header (Disponível/Pausado global em todos os deptos do user) + badge `"Em fila — Lucas (3:42)"` em `ConversationItem` com countdown ao vivo (filtra próprio assignee) + cancelar queue_event ativo em `assignAgent` (manual_override). Smoke Playwright: badge renderizou + countdown decrementou 12s em 5s real + toggle persiste no DB. tsc 0, vitest 662 (5 pré-existentes). |
...
| **Dashboard do Gestor unificado — Fase 1** | ✅ Shipped (2026-05-11, v7.33.0) | Aposenta abas Métricas/Insights, consolida em 4 zonas (Pulso/Tendência/Atendimento/IA-Comercial) com motivos de contato + horário das conversas absorvidos do DashboardHome. Coluna `instances.is_sandbox` esconde Sandbox IA do gerente (toggle só pra super_admin). Novo gráfico Leads Novos vs Recorrentes via RPC `get_leads_new_vs_returning`. Acesso do role `gerente` confirmado via `CrmRoute`. |
| **Dashboard do Gestor — Fase 2 (avançado)** | ✅ Shipped (2026-05-11, v7.34.0) | 4 RPCs novas: `get_response_time_percentiles` (P50/P95), `get_abandoned_conversations` (sem resposta 24h), `get_demand_vs_coverage_by_hour` (gap equipe), `get_conversion_by_origin` (taxa por canal). 4 cards: ResponseTimeCard, AbandonedConversationsList (com link pro helpdesk), DemandVsCoverageChart (ComposedChart), ConversionByOriginCard. Hook único `useManagerAdvancedMetrics` Promise.all. Validado Eletropiso 30d: P50=23s, 6 conversas paradas. |
| **Dashboard do Gestor — Fase 3 (pivô comercial)** | ✅ Shipped (2026-05-11, v7.35.0) | Tirado: cards/colunas/metas de Custo IA. Adicionado: 2 RPCs (`get_unanswered_first_messages`, `get_active_quotes`); componente genérico `PendingConversationsCard` (substituiu `AbandonedConversationsList`); Zona 3 reorganizada em 3 linhas (pendências críticas em 3 cols + objeções/motivos + equipe). Top Objeções promovido do InsightsTab para a Zona 3. Eletropiso 30d: 1 lead sem 1ª resposta há 716h. Fase 4 backlog: drill-down, comparação período-vs-período, alertas WhatsApp pessoais. |
| **Dashboard de Fila — "Sem atendimento" + ver/reatribuir (mobile)** | ✅ Shipped (2026-05-31, v7.63.0) | `/dashboard/fila` (`QueueDashboard`) ganha 3 abas (Sem atend./Ao vivo/Atendentes). Aba "Sem atendimento": leads que a IA transbordou (`status_ia='shadow'`) e o atendente atribuído ainda **não respondeu** — 👁 Ver (modal read-only que distingue IA vs Atendente + "Abrir no Helpdesk") e ↪ Reatribuir (RPC `manager_reassign_conversation` role-gated, fila-coerente). RPC `get_unattended_handoff_leads` (resposta humana por `sender_id` web OU `+90s` celular; exclui ponte-handoff/OOF). Só EletropisoV2 (`useManagerInstances`), tempo com dias (`formatWaiting`). Migration `20260531000000`. Detalhe: memória `project_manager_attendance_dashboard`. |
