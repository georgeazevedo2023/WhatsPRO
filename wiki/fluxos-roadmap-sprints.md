...
### S9: Validator + Metrics + Shadow ✅ COMPLETO (2026-04-12)

**`services/validator.ts` (230 linhas):** 10 checks automáticos (0 tokens): size, language (heurística PT), prompt leak (13 patterns), price (vs catálogo ±10%), repetition (vs last_response), greeting repeat, name frequency (max 1x), emoji (max 5), markdown artifacts, PII (CPF/email/tel). 3 ações: pass/correct/block. 3 falhas → handoff. corrected_text aplicado no send.

**`services/metrics.ts` (55 linhas):** createTimer → mark(6 labels) → finalize → TimerBreakdown + CostBreakdown. Salvo em flow_events.timing_breakdown + cost_breakdown (colunas JSONB dedicadas).

**Shadow Mode:** `flows.mode === 'shadow'` → pipeline roda (intent+subagent+validator) mas NÃO envia. Loga via flow_events (shadow_extractions = S11 batch). Response: `shadow:true, message_sent:false`.

**4 arquivos modificados:** types.ts, stateManager.ts (logFlowEvent timing/cost), services/index.ts, index.ts. **4 fixes da auditoria** aplicados.

**E2E:** Normal (timing 3481ms, breakdown no DB ✅) | Shadow (message_sent:false ✅) | 0 migrations.
...
### S11: Conversa Guiada + FlowEditor Completo ✅ COMPLETO (2026-04-12, commit 15007ff)

**Edge function `guided-flow-builder`:** Chat admin → gpt-4.1-mini com `response_format: json_object` → `draft_flow` JSON + `suggestions`. Sessão persistente em `guided_sessions` (TTL 24h).
**Migration:** `20260416000001_guided_sessions.sql` — tabela `guided_sessions` (messages JSONB, draft_flow JSONB, expires_at — 24h, pg_cron cleanup 02:00)
**FlowEditor — 5 tabs:** Identidade | Gatilhos | **Subagentes** (funcional — CRUD + drag-and-drop @dnd-kit) | **Inteligência** (KPIs + top intents + validator stats) | Publicar
**Novos componentes:** `FlowStepsPanel` (SortableContext) | `StepConfigForm` (8 tipos) | `FlowIntelPanel` | `GuidedFlowBuilderModal` (chat UI)
**Novos hooks/types:** `useFlowSteps` (5 mutations) | `SubagentType` | `SUBAGENT_TYPE_LABELS` | `FlowStep`
**Auditoria pré-execução:** 3 issues críticos resolvidos antes do código (tipos duplicados, re-export desnecessário, card já existia disabled)
...
## Cobertura: 14 tabelas ✅ | 13 params ✅ | 8 subagentes (4/8 reais) ✅ | 5 serviços (4/5 reais) ✅ | Feature flag ✅ | Types+UI ✅
