...
const Leads = () => {
  ...
  const clearContextMutation = useMutation({
    mutationFn: async ({ lead }: { lead: LeadData }) => {
      ...
      if (convIds.length > 0) {
        await supabase.from('conversations').update({ tags: [clearedTag], ai_summary: null, status_ia: STATUS_IA.LIGADA, lead_msg_count: 0, human_handling_at: null }).in('id', convIds);
